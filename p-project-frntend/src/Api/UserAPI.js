...
const fetchAllAlumni = async () => {
  let users = await fetch(`http://127.0.0.1:8000/alumni/user/`)
  .then(res => res.json())
  .then(data => data)
  return users
}
...
const fetchEvent = async () => {
  let eventlist = await fetch(`http://127.0.0.1:8000/alumni/events/`)
  .then(res => res.json())
  .then(data => data)
  return eventlist
}
...
const fetchMessage = async () => {
  let message = await fetch(`http://127.0.0.1:8000/user/messages/`)
  .then(res => res.json())
  .then(data => data)
  return message
}
